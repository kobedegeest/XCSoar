...
areas with thick borders.  The colour of the areas is specific to
different airspace categories and may be configured by the
...
Airspace areas are filled with a transparent colour so terrain and
topography remain visible.  Where airspaces overlap, all borders are
drawn.  Fill padding is a continuous band inside the airspace boundary,
keeping overlapping borders visible.
...
\subsection*{NOTAM airspace}\label{sec:notam-airspace}
...
\emph{Map Display / Airspace / NOTAM}.  When enabled and a valid
...
\begin{itemize}
\item IFR-only NOTAMs,
\item NOTAMs which are not currently effective,
\item NOTAMs with an affected radius larger than 100~km, and
\item NOTAMs with the Q-code prefixes \texttt{QA}, \texttt{QK},
  \texttt{QN}, \texttt{QOL}, \texttt{QOA} and \texttt{QOBTT}
...
filter, and the \button{List} button can be used to review the current
NOTAM list.  Use \button{Refresh} to manually request a new download for
...
When an airspace region is acknowledged, the region is drawn as an
outline only.
