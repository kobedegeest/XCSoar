 ...
 *
 * Compile with:
 *   gcc -Wall add-connection-libnm.c -o add-connection-libnm `pkg-config --libs
 * --cflags libnm`
 */
...
static void added_cb(GObject *client, GAsyncResult *result,
                     gpointer user_data) {
...
static void add_connection(NMClient *client, GMainLoop *loop,
                           const char *con_name) {
  ...
  g_object_set(G_OBJECT(s_con), NM_SETTING_CONNECTION_UUID, uuid,
               NM_SETTING_CONNECTION_ID, con_name, NM_SETTING_CONNECTION_TYPE,
               "802-3-ethernet", NULL);
  g_free(uuid);
